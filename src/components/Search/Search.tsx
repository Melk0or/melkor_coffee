...
const Search: React.FC = () => {
    const valueOfSearchInput: string = useSelector(
        getValueOfSearchInputSelector
    );
    //диспач
    const dispatch = useDispatch();
    //хук для доступа к эелменту

    const inputEl = useRef<HTMLInputElement | null>(null);
    // console.log(inputEl);

    //очищаем инпут и фокусим на инпуте
    const clearInput = () => {
    ...
    const updateReqInput = useCallback(
    ...
    const onChangeInput = (event: ChangeEvent<HTMLInputElement>) => {
        dispatch(setValueOfSearchInput(event.target.value));
        updateReqInput(event.target?.value);
    };
